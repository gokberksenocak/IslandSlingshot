...
public class BallManager : MonoBehaviour
{
    ...
    void WinCheck()
    {
        if (_aiController.DefeatedEnemyCount == _aiController.LevelNPCCount)
        {
            ...
            _uiManager.LevelTextAtEnd.text = "LEVEL " + PlayerPrefs.GetInt("LevelIndex").ToString() + " CLEARED!";
            _uiManager.UIPanel.SetActive(false);
            _uiManager.InputPanel.SetActive(false);
            _uiManager.WinPanel.SetActive(true);
            _uiManager.WinPanel.transform.DOMove(_uiManager.ExamplePanel.transform.position, 1.5f).OnComplete(() =>
            {
